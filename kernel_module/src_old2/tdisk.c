...
static void td_request_chunk_finished(void *data, unsigned int bytes, int success);
...
static int __init td_init(void)
{
	...
	td_dev = tdisk_allocate(512, 512, 4);
	if(td_dev == NULL)
	{
		return -ENOMEM;
	}
...
static void td_request_chunk_finished(void *data, unsigned int bytes, int success)
{
	...
	if(!__blk_end_request_cur(req, success))
	{
...
static void td_request(struct request_queue *q)
{
    ...
    while ((req = blk_fetch_request(q)) != NULL)
    {
		...
		sector_t start_sector = blk_rq_pos(req) * 512;
		unsigned int sector_count = blk_rq_sectors(req) * 512 / td_dev->sector_size;
		...
		if(__div64_32(&start_sector, td_dev->sector_size) != 0)
			printk(KERN_ERR "tDisk: bio start sector (%llu) and tdisk sector size (%d) don't match. Data truncation possible\n", blk_rq_pos(req), td_dev->sector_size);
