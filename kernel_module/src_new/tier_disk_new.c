...
int make_disk_requests(struct td_device *td_dev, MBdeque *requests, struct td_callback_data *data, int direction, sector_t first, sector_t last, u8 *buffer, unsigned int length)
{
	...
	for(i = 0; i < requests->count; ++i)
	{
		...
		//Process requests
		printk(KERN_ERR "tDisk: %u requests for physical disk (%u bytes)\n", current_requests->count, current_requests->count * td_dev->sector_size);
		for(j = 0; j < current_requests->count; ++j)
		{
			index = MBdeque_get_at(current_requests, j);

			//printk(KERN_DEBUG "tDisk: Page: off=%llu size=%u\n", (index->logical_sector-first) * td_dev->sector_size, td_dev->sector_size);

			if(j == 0)
			{
				//TODO: convert MBdeque current requests to one
				//single request struct which counts the number of sectors
				printk(KERN_DEBUG "tDisk: Page: off=%llu size=%u\n", (index->logical_sector-first) * td_dev->sector_size, current_requests->count * td_dev->sector_size);

				if(!bio_add_page(bio_req, virt_to_page(buffer/* + (index->logical_sector-first) * td_dev->sector_size*/), current_requests->count * td_dev->sector_size, 0))
				{
					printk(KERN_ERR "tDisk: bio_add_page failed!\n");
				}
				printk(KERN_DEBUG "tDisk: Page added\n");
			}
		...
		printk(KERN_DEBUG "tDisk: Physical disk request: Last logical sector: %llu, Physical-Start: %llu, Physical-End: %llu, Actual length (%u/%u bytes) Request: %u/%llu\n",
													index->logical_sector,
													bio_req->bi_iter.bi_sector,
													bio_end_sector(bio_req),
													bio_req->bi_iter.bi_size, length,
													i+1, last-first+1);
